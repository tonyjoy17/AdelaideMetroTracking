...
function renderSidebar() {
  ...
  renderVList(scroll);
  renderMobileSummarySheet();
}

function mobileSummaryStats() {
  const counts = { all: S.vehicles.length, tram: 0, train: 0, bus: 0, alerts: 0, moving: 0 };
  S.vehicles.forEach(v => {
    if (counts[v.routeType] !== undefined) counts[v.routeType]++;
    if (vehicleAlertCount(v)) counts.alerts++;
    if ((v.speed || 0) > 0.5) counts.moving++;
  });
  return counts;
}

function mobileSummarySubtitle(stats) {
  if (!stats.all) return 'Waiting for live vehicles to load';
  if (stats.alerts) return `${stats.alerts} active alert${stats.alerts === 1 ? '' : 's'} across the network`;
  return `${stats.moving} vehicle${stats.moving === 1 ? '' : 's'} currently moving`;
}

function renderMobileSummarySheet() {
  if (!isMobile()) return;
  if (S.selectedId || S.selectedStop || S.mode !== 'list' || S.tab !== 'all' || S.filterRoute) return;
  ...
  const stats = mobileSummaryStats();
  const feedTime = document.getElementById('feed-time')?.textContent || 'LIVE';
  const activeFilter = getActiveFilterMeta();
  const highlight = activeFilter
    ? `Showing ${activeFilter.label.toLowerCase()}`
    : `Live network snapshot`;
  const html = `<div class="m-summary">
    <div class="m-summary-hero">
      <div>
        <div class="m-summary-kicker">${highlight}</div>
        <div class="m-summary-title">${stats.all ? `${stats.all} live services on the map` : 'Loading live services'}</div>
        <div class="m-summary-sub">${mobileSummarySubtitle(stats)}</div>
      </div>
      <div class="m-summary-live">
        <span class="m-summary-live-dot"></span>
        <span>Updated ${feedTime}</span>
      </div>
    </div>
    <div class="m-summary-grid">
      <button class="m-summary-stat" onclick="setTab('tram'); mobileShowFilteredList();">
        <span class="m-summary-stat-label">Trams</span>
        <strong>${stats.tram}</strong>
      </button>
      <button class="m-summary-stat" onclick="setTab('train'); mobileShowFilteredList();">
        <span class="m-summary-stat-label">Trains</span>
        <strong>${stats.train}</strong>
      </button>
      <button class="m-summary-stat" onclick="setTab('bus'); mobileShowFilteredList();">
        <span class="m-summary-stat-label">Buses</span>
        <strong>${stats.bus}</strong>
      </button>
      <button class="m-summary-stat" onclick="setTab('alerts'); mobileShowFilteredList();">
        <span class="m-summary-stat-label">Alerts</span>
        <strong>${stats.alerts}</strong>
      </button>
    </div>
    <div class="m-summary-actions">
      <button class="m-summary-btn" onclick="mobDrawerOpen()">Browse vehicles</button>
      <button class="m-summary-btn" onclick="showNearby()">Nearby stops</button>
      <button class="m-summary-btn" onclick="openPlanner()">Plan trip</button>
    </div>
  </div>`;

  title.textContent = 'Live Overview';
  setHtmlIfChanged(content, html);
  detail.classList.add('open');
  detail.classList.remove('expanded', 'follow-compact');
  syncControlState();
}
